...
window.antoraLunr = (function (lunr) {
  ...
  function search (index, text) {
    ...
    text = text.trim(" ") // So "<TERM> " is the same as "<TERM>"
    // https://github.com/olivernn/lunr.js/issues/287#issuecomment-320767924
    return index.query(function (q) {
      // Multiple terms
      if (text.includes(" ")) {
        text.split(" ").forEach( t => {
          // I guess this is expected, that you get first AND and the OR
          q.term(t, { boost: 200, usePipeline: false, presence: lunr.Query.presence.REQUIRED })
          q.term(t, { boost: 1, usePipeline: false, presence: lunr.Query.presence.OPTIONAL })
        }
        )
      // Single term
      } else {
        // exact first, then wildcards, then similar words
        q.term(text, { boost: 200, usePipeline: false })
        q.term(text, { boost: 20, usePipeline: false, wildcard: lunr.Query.wildcard.TRAILING })
        q.term(text, { boost: 5, usePipeline: false, editDistance: 1 })
      }
    })
  }
  ...
  function searchIndex (index, store, text) {
    ...
    console.log(`Searching for ${text}, got ${result.length} results.`)
